...
/// }
///
/// fn greet_with_result(name: String) -> Result<String, String> {
///   if name.is_empty() {
    ...
    Builder::new("router")
        .register_asynchronous_uri_scheme_protocol("router", move |context, request, responder| {
            ...
            let response = match *request.method() {
                ...
                Method::POST => {
                    let mut response = router.handle_request(
                        context.app_handle(),
                        context.webview_label(),
                        request,
                    );
                    let headers_mut = response.headers_mut();
                    headers_mut.insert(ACCESS_CONTROL_ALLOW_ORIGIN, HeaderValue::from_static("*"));
                    headers_mut.insert(ACCESS_CONTROL_ALLOW_HEADERS, HeaderValue::from_static("*"));
                    response
                }
